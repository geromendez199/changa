...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    const { valid, errors } = validateForm(values, {
      ...
      password: [v => notEmpty(v), 'Ingres├í tu contrase├▒a.'],
    });
    ...
    try {
      ...
      Alert.alert('No se pudo ingresar', e.message || 'Revis├í tus datos e intent├í nuevamente.');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          <View style={[s.center, isNarrow && s.centerNarrow]}>
            ...
            <View style={[s.hero, isNarrow && s.heroNarrow]}>
              ...
              <Text style={[s.logoSub, isNarrow && s.logoSubNarrow]}>RAFAELA ┬À SANTA FE</Text>
            </View>
            ...
            <View style={[s.card, isNarrow && s.cardNarrow]}>
              ...
              <Text style={s.sub}>Ingres├í para continuar y contratar prestadores en minutos.</Text>

              <Field label="Email"      value={email}    onChangeText={setEmail}    placeholder="tu@email.com"  keyboard="email-address" />
              <Field label="Contrase├▒a" value={password} onChangeText={setPassword} placeholder="ÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇó" secure />

              <Btn label="Ingresar ÔåÆ" onPress={handleLogin} loading={loading} />
            ...
            {/* Footer */}
            <Text style={[s.footer, isNarrow && s.footerNarrow]}>Hecho en Rafaela ┬À Experiencia r├ípida, simple y segura</Text>
          </View>
