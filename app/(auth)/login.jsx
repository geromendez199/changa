...
export default function Login() {
  ...
  return <SafeAreaView style={{ flex: 1, padding: 20, justifyContent: 'center', maxWidth: 500, alignSelf: 'center' }}><Text style={{ fontSize: 40, fontWeight: '800', color: '#6C47FF' }}>changa.</Text><Input label='Email' value={email} onChangeText={setEmail} keyboardType='email-address' /><Input label='Contraseña' value={password} onChangeText={setPassword} secureTextEntry /><Button title='Iniciar sesión' onPress={submit} loading={loading} /><Link href='/(auth)/forgot-password' style={{ marginTop: 12, color: '#6C47FF' }}>Olvidé mi contraseña</Link><Link href='/(auth)/register' style={{ marginTop: 8 }}>Crear cuenta</Link></SafeAreaView>;
}
