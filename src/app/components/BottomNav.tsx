...
export function BottomNav() {
  ...
  const navItems = [
    ...
    { path: "/publish", icon: Plus, label: "Publicar", isCenter: true },
    { path: "/my-jobs", icon: Briefcase, label: "Trabajos" },
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              key={item.path}
              onClick={() => navigate(item.path)}
              className="flex flex-col items-center justify-center gap-1.5 py-2 px-3 rounded-2xl transition-all duration-200 min-w-[64px]"
            >
              <item.icon
                size={24}
                className={`transition-colors ${
                  isActive(item.path) ? "text-[#10B981]" : "text-gray-400"
                }`}
                strokeWidth={isActive(item.path) ? 2.5 : 2}
              />
              <span
                className={`text-[10px] font-medium transition-colors ${
                  isActive(item.path) ? "text-[#10B981]" : "text-gray-500"
                }`}
              >
                {item.label}
              </span>
